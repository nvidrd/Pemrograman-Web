...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_POST['name'], $_POST['rating'], $_POST['content'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO reviews3 (page_id, name, content, rating, submit_date) VALUES (?,?,?,?,NOW())');
    ...
    $stmt = $pdo->prepare('SELECT * FROM reviews3 WHERE page_id = ? ORDER BY submit_date DESC');
    $stmt->execute([$_GET['page_id']]);
    $reviews3 = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the overall rating and total amount of reviews
    $stmt = $pdo->prepare('SELECT AVG(rating) AS overall_rating, COUNT(*) AS total_reviews FROM reviews3 WHERE page_id = ?');
    $stmt->execute([$_GET['page_id']]);
...
<?php foreach ($reviews3 as $review3): ?>
<div class="review">
    <h3 class="name"><?=htmlspecialchars($review3['name'], ENT_QUOTES)?></h3>
    <div>
        <span class="rating"><?=str_repeat('&#9733;', $review3['rating'])?></span>
        <span class="date"><?=time_elapsed_string($review3['submit_date'])?></span>
    </div>
    <p class="content"><?=htmlspecialchars($review3['content'], ENT_QUOTES)?></p>
</div>
<?php endforeach ?>