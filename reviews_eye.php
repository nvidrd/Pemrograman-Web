...
if (isset($_GET['page_id'])) {
    ...
    if (isset($_POST['name'], $_POST['rating'], $_POST['content'])) {
        ...
        $stmt = $pdo->prepare('INSERT INTO reviews2 (page_id, name, content, rating, submit_date) VALUES (?,?,?,?,NOW())');
    ...
    $stmt = $pdo->prepare('SELECT * FROM reviews2 WHERE page_id = ? ORDER BY submit_date DESC');
    $stmt->execute([$_GET['page_id']]);
    $reviews2 = $stmt->fetchAll(PDO::FETCH_ASSOC);
    // Get the overall rating and total amount of reviews
    $stmt = $pdo->prepare('SELECT AVG(rating) AS overall_rating, COUNT(*) AS total_reviews FROM reviews2 WHERE page_id = ?');
    $stmt->execute([$_GET['page_id']]);
...
<?php foreach ($reviews2 as $review2): ?>
<div class="review">
    <h3 class="name"><?=htmlspecialchars($review2['name'], ENT_QUOTES)?></h3>
    <div>
        <span class="rating"><?=str_repeat('&#9733;', $review2['rating'])?></span>
        <span class="date"><?=time_elapsed_string($review2['submit_date'])?></span>
    </div>
    <p class="content"><?=htmlspecialchars($review2['content'], ENT_QUOTES)?></p>
</div>
<?php endforeach ?>